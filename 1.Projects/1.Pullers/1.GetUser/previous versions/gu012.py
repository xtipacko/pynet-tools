...
def printinfo(result, username, password):
    for bras_excerpt in result:
        ...
        print('  Tunnel Remote IP : {ip} (VPN {purpose})'.format(ip         = tunnelremoteip,
        ...
        print('  Session time     : {sessiontime}\n'
              ...
              '  Tunnel Source IP : {tunelsourceip}'. format(**vpdn_tun_info_dict), end='')
        nexthop_to_source = get_nexthop(vpdn_tun_info_dict['tunelsourceip'],session)
        print(' (Next Hop is {nexthop})'.format(nexthop=nexthop_to_source))
        ...
        client_mac_address = get_client_mac(vpdn_tun_info_dict['tunelsourceip'], 
                                            ...
                                            adminusername, 
                                            password)
        print('  MAC Address      : {mac}'.format(mac=client_mac_address))
        client_vendor = mac_lookup(client_mac_address)
        print('  Vendor           : {vendor}'.format(vendor=client_vendor))
        session.disconnect()
...
def get_client_vendor(client_mac_address):
    pass
