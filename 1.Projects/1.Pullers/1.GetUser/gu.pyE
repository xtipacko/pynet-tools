...
from netconnect import ConnectHandler
...
def vpn_subnet_identifier(tunnelremoteip, vpn_subnets):
    try:
        ...
        if IPv4Address(tunnelremoteip) not in IPv4Network('10.0.0.0/8'):
            if tunnelremoteip != '0.0.0.0':
                return  Fore.YELLOW + 'Public' + Fore.RESET
            else:
                return  Fore.MAGENTA + 'Public' + Fore.RESET
...
def parse_show_vpdn_ses(command_output):
    matchobj = re.match(r'[\s\S]*Internet\ Address\ is\ (?P<tunelsourceip>.*)[\s\S]*'
                        r'Time\ since\ change\ (?P<sessiontime>.*),'
                        r'\ interface\ Vi(?P<interfacename>.*)[\s\S]*', command_output)

    mdict = matchobj.groupdict()

    vpdn_tun_info_dict = {'tunelsourceip': mdict.get('tunelsourceip', 'Unknown'),
                          'sessiontime'  : mdict.get('sessiontime', 'Unknown'),
                          'interfacename': 'Virtual-Access'*bool('interfacename' in mdict) + 
                                                   mdict.get('interfacename', 'Unknown') }
...
def get_client_mac(tunnelsource, l3ipaddr, adminusername, password):
    ...
    try:
        ...
        print(Fore.RED, f'Connection to {l3ipaddr} Refused', Fore.RESET)
    except:
        print(Fore.RED, f'Cannot get client mac from {l3ipaddr}', Fore.RESET)
...
def parse_show_ip_arp(command_output):
    ...
    if matchobj:#tmp
        ...
        return 'IP is absent in arp table'

if __name__ == '__main__':
    #entry point here
    initcolor()
    print('Get User v0.15')
    if len(sys.argv) < 2:
        print('\n') 
